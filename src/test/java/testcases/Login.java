...
String ofcName;
	...
	public void clickonMyworkspace() throws InterruptedException {
		...
		List<WebElement> ofc=driver.findElements(By.xpath("//div[@class='ui-menu-item-wrapper']"));
		System.out.println("offices in required Location:"+ofc.size());
		for(WebElement l:ofc) {
			System.out.println(l.getText());
			if(l.getText().equals("HYD DLF Block 1")) {
				ofcName=l.getText();
				break;
			}
		}
//		Scanner sc=new Scanner(System.in);
//		System.out.println("Enter office name that you want to book from above list:");
//		ofcName=sc.nextLine();
		System.out.println("selected ofc:"+ofcName);
		
		driver.findElement(By.xpath("//div[@class='ui-menu-item-wrapper' and text()=\""+ofcName+"\"]")).click();
	...
	public void selectSeat() {
		...
		System.out.println("Total available Seats in "+ofcName+":"+ availableSeats.getText());
		...
		List<WebElement> ListofSeats=driver.findElements(By.xpath("//div[@data-building=\""+ofcName+"\" and @data-wing=\"B\"]"));
		System.out.println("seats in required wing:"+ListofSeats.size());
		for(WebElement l:ListofSeats) {
			System.out.println(l.getText());	
		}
		Scanner sc=new Scanner(System.in);
		System.out.println("Enter seat No that you want to book from above list:");
		String seatNo=sc.next();
		for(WebElement m:ListofSeats) {
		if(m.getText().contains(seatNo)) {
			
			WebElement checkBox=driver.findElement(By.xpath("//input[@class='check-mark' and @id='"+seatNo+"']"));
			js.executeScript("arguments[0].scrollIntoView();", checkBox);
	        js.executeScript("window.document.getElementById('"+seatNo+"').click()");
		 }
		}
